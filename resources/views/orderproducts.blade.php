...
<body>
    ...
    <div class="box">
        <div class="bix">
            ...
            <br>
            @foreach ($orderprods as $orderpro)
                <div class="bixer">
                    <div> {{ $orderpro->catalog->name }} </div>
                    <div> {{ $activationCode }} </div><br>
                    @if ($orderpro->catalog->category == 'steam')
                        <a href="{{ route('render-steam-activation') }}" class="introd">Инструкция по использованию</a>
                    @elseif($orderpro->catalog->category == 'playstation')
                        <a href="{{ route('render-playstation-activation') }}" class="introd">Инструкция по использованию</a>
                    @elseif($orderpro->catalog->category == 'xbox')
                        <a href="{{ route('render-xbox-activation') }}" class="introd">Инструкция по использованию</a>
                    @elseif($orderpro->catalog->category == 'epicgames')
                        <a href="{{ route('render-epicgames-activation') }}" class="introd">Инструкция поиспользованию</a>
                    @elseif($orderpro->catalog->category == 'gog')
                ...
                <table class="tableorder123">
                    <tr>
                        ...
                        <td class="te1">{{ $orderpro->order_id }} (выписан: {{ $orderpro->created_at }}) на сумму
                            {{ $orderpro->price }} ₽</td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                        ...
                        <td class="te1">{{ $orderpro->catalog->name }}</td>
                    </tr>
